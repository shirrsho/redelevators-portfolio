...
| 2 | **Services index + 8 service pages** | SEO + depth | **Index built and live** (`/services`), rebuilt 12 Sep 2026 around the real two-pillar taxonomy (Marketing ×4, AI Automation ×4) from the company's own portfolio decks — see `PRODUCT.md` → *Evidence on Hand*. Individual detail pages use the template from `brief-service-page.md` (`Handover.tsx`, `PageHeader.tsx`) but none has content yet; `serviceDetails` is empty, so `/services/[slug]` correctly generates zero pages. Real case-study content (named clients, screenshots, metrics) exists but is **explicitly not cleared for the public site** — build detail-page content from generic per-service material only, never from the decks' client specifics, until told otherwise. |
...
| 4 | **Systems / Teardowns** | Case-study substitute | Anonymized, mechanism-led, honest. Becomes real case studies later when permission and numbers exist. **Note (12 Sep 2026):** the home page's `systems` data (`src/lib/content.ts`) still labels its three chains "Sales & follow-up" / "Ops & reporting" / "Marketing automation" — names that don't match the rebuilt 8-service taxonomy. Left untouched this round (out of scope for the taxonomy rebuild); reconcile when this page is built. |
